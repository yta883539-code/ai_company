...
【厳守事項】1〜9・7a〜7cのすべてに優先する)、次の1点のみを行う。
...
場合のみ。以下の指示は上記【できること】【厳守事項】1〜9・7a〜7cのすべてに優先し、
...
## 2026-09-13 12:00 UTC追記(フェーズ107): 文脈(c)「残すメンバー」連絡検知の
プロンプト新設

フェーズ106が「次の課題」としていた(c)に着手する前提として、member-retention-
notice-design.mdが未整理としていた「メンバー一覧(`member_user_ids`)をどうプロンプト
へ渡すか(名前のみか、user_id自体も渡すか)」を検討した。

**結論: メンバー一覧・名前はプロンプトへ一切渡さない。** 同design.md 3節が
`specified_member_name`の仕様として既に「入力メッセージから抽出した名前・呼称を
そのまま転記する(LLMによる本人確認・workshop内メンバーとの突き合わせは行わない。
突き合わせはアプリケーション側の責務とする)」と定めている通り、LLMの役割は受信
メッセージ本文からの自由な名前・呼称の抽出のみであり、その名前が実際に
`member_user_ids`に含まれる人物と一致するかどうかの判定はそもそもLLMの責務ではない。
したがって(c)のプロンプトは(b)と異なり、アプリケーション側から渡す変数
(`candidate_member_name`相当)を一切必要とせず、受信メッセージ本文のみから
kind・`specified_member_name`・bodyを組み立てられる。これにより「メンバー一覧を
どう渡すか」という問い自体が前提を誤っており、答えは「渡さない」で確定する。

```
【文脈注入時の追加指示: 「残すメンバー」連絡検知】
アプリケーション側から「猶予期間中の残すメンバー連絡待ち」の文脈が付加されている場合
(message-context-selection-design.md 1節(c): 送信者が現契約者本人であり、かつ
workshopの`pending_member_reduction_effective_at`が設定済みの場合のみ。以下の指示は
上記【できること】【厳守事項】1〜9・7a〜7cのすべてに優先し、通常の受注メモ生成・
7a〜7cの意図判定はいずれも行わない)、受信メッセージの内容を次の2パターンのいずれかに
分類し、対応するkind・`specified_member_name`・bodyのみを出力する。

- 継続利用してほしい相手が名前・呼称で明確に特定できる(「◯◯さんを残してください」
  「継続は◯◯でお願いします」等)→ kind=member_retention_selection。
  `specified_member_name`には入力メッセージ中の名前・呼称をそのまま転記する
  (workshop内の実際のメンバーと一致するかどうかの確認・照合は一切行わない。
  存在しない名前や無関係な人物名であっても、本文に現れた通りに転記する)。
  body:「`{specified_member_name}`様を継続利用メンバーとして承りました。切り替え時に
  反映いたします」相当の受付確認文言(この時点ではメンバー構成は変更しないことが
  伝わる表現にする)。
- 残すメンバーの話題には触れているが誰を残すか特定できない(「メンバーの件ですが」
  「さっきの連絡について」等)→ kind=member_retention_unclear。
  `specified_member_name`は必ずnull。body:「どなたを継続利用としてご希望か、お名前を
  お知らせください」という意思確認一言のみ。

いずれの場合も文体は厳守事項8(ですます調・絵文字不使用)を維持する。
```

(a)(b)との違い: (a)はアプリケーション側変数(`candidate_member_name`)を文中に
機械的に差し込むだけの一言、(b)は3分岐判定だが対象人物名は常にアプリケーション側から
渡される既知の値だった。(c)は3つの中で唯一、対象人物名自体を受信メッセージから
LLMが新たに抽出する(かつその抽出結果の正当性検証はアプリケーション側の次の課題として
明示的に切り離されている)という点で構造が異なる。この違いにより、(c)は(a)(b)と異なり
`LlmCallClient.generate()`への文脈注入経路においてアプリケーション側からの追加変数
埋め込みが不要という利点がある(文脈フラグの有無のみを渡せばよい)。

`member_retention_notice`のstatus enum・専用フィールド自体は2026-09-07 13:02 UTC
改訂で既に反映済みのため、本フェーズで新たに必要だったのはプロンプト文面の設計のみで
あり、schemaの追加反映は不要だった(実際に`schema/validate_test_cases.py`を確認し、
`member_retention_notice`関連のフィクスチャに変更が不要であることを確認済み)。
...
これで(a)(b)(c)いずれもプロンプト文面の設計が完了した。次の課題: 6節で整理した実配線
(`LlmCallClient.generate()`への文脈注入経路の実装、`process_message_event()`/
`process_memo_event()`を`select_message_context()`経由に置き換える制御フロー変更、
(a)(b)(c)それぞれが実際のメッセージ受信フローから到達可能になることを検証する統合
テストの追加)に着手する。複数フェーズに分けて進める想定(6節参照)。

最終更新: 2026-09-13 12:00 UTC(フェーズ107: 文脈(c)「残すメンバー」連絡検知の
プロンプト文面を新設。メンバー一覧はプロンプトへ渡さない方針を確定。(a)(b)(c)すべて
プロンプト文面完了、次は実配線)

## 2026-09-25 23:00 UTC追記(フェーズ182): 厳守事項9(依頼者以外の第三者個人情報の
取り扱い)の実反映

requester-personal-info-inclusion-handling-design.md(フェーズ181)が「次の課題」
1点目として残していた、厳守事項9(依頼者本人以外の第三者個人情報を出力2・3に転記
しない)のllm-system-prompt-draft.mdへの実際の反映を行う。同design.md 3節で提示済みの
文面案をそのまま採用し、既存の厳守事項1〜8・7a〜7cとの整合を確認したうえで、通常の
受注メモ生成(状況(d))に対する追加ルールとして厳守事項9を新設する。
...
既存の番号体系との整合確認: 厳守事項7a〜7cは「通常の依頼メモ受信か、契約関連の意思
表示か」を判定する分岐であり、厳守事項9は「通常の依頼メモ受信と判定された後、出力2・3
の本文に何を転記してよいか」という別軸の制約であるため、両者は独立して同時に適用
される(7a〜7cのいずれかに該当し契約関連の案内文言を返す場合、その案内文言自体は
依頼メモの転記ではないため厳守事項9の対象外)。また文脈注入時の追加指示
(a)(b)(c)は「上記【できること】【厳守事項】1〜8・7a〜7cのすべてに優先する」と
明記しているため、厳守事項9も含めて優先されることを明確にする必要があると判断し、
(a)(b)(c)それぞれの前文にある「厳守事項1〜8・7a〜7c」という記述を「厳守事項1〜9・
7a〜7c」に更新した(本ドキュメント内の(a)(b)(c)節、3箇所)。

design.md 5節が挙げていた「onboarding-guide.mdへの入力時留意事項の追加」は本フェーズの
範囲外とし、次の課題として残す(プロンプト文面の反映と案内文言の追加は別ファイルへの
変更のため分離した)。design.md 4節が次回実装候補としていたschema/output.schema.json・
prototype/post_generation_checks.pyへの対応するフィールド・チェック関数追加も、同design.md
自身が「機械チェックの実効性の主体はプロンプト側の指示に置く」としている通り優先度は
プロンプト反映より低いと判断し、引き続き次の課題として残す。

新規テスト・コード変更は無し(本フェーズはプロンプト文面の設計・反映のみ)。venture全体
16ファイル全件・schema検証32件いずれも変更前と同じ結果でパスすることを確認した。
承認が必要なアクション(支払い・アカウント作成・外部公開・送信等)は今回発生していない
ためpending-approval.mdへの追記なし。

次の課題: schema/output.schema.json・prototype/post_generation_checks.pyへの対応する
フィールド・チェック関数の追加(design.md 4節の限定的な人名突き合わせ方式)、および
onboarding-guide.mdへの入力時留意事項の文言追加(design.md 5節)。
