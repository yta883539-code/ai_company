...
## 9. 追記(フェーズ107): (c)「残すメンバー」連絡検知のメンバー一覧受け渡し方針の確定

8節が(c)着手の前提としていた「メンバー一覧(`member_user_ids`)をどうプロンプトへ渡すか
(名前のみか、user_id自体も渡すか)」を検討した結果、**メンバー一覧・名前はプロンプトへ
一切渡さない**方針で確定した(詳細な理由はllm-system-prompt-draft.md 2026-09-13 12:00 UTC
追記参照)。

member-retention-notice-design.md 3節が`specified_member_name`の仕様として既に
「LLMによる本人確認・workshop内メンバーとの突き合わせは行わない」と定めていた通り、
LLMは受信メッセージ本文からの自由な名前・呼称抽出のみを担当し、実際のメンバー一覧との
突き合わせは同4節が明示する通りアプリケーション側の次の課題として引き続き切り離されて
いる。したがって「メンバー一覧をどう渡すか」という問い自体が、(c)がそもそも一覧を必要と
しないという前提の見落としであったことが判明した。
