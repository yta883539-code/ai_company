...
## 11.10 追記(フェーズ105): (a)(b)(c)返信文面生成方針の決定・(a)プロンプト新設

11.9節がmessage-context-selection-design.md 6節へ委ねた「次の課題1.((a)(b)(c)の
返信文面生成方針の決定)」に対応した。結論・詳細はmessage-context-selection-design.md
7節に記載したため要旨のみ記す。

- 方針: (a)(b)(c)いずれも既存方針(LLM構造化出力への委任)を維持し、`member_limit_
  reached`(11.8節)のようなPython側決定論的テンプレートには切り替えない。(b)(c)は
  自由記述の自然文からの意図解釈が必須でPython側ヒューリスティックでは対応できないため、
  (a)はtone-and-manner-guideline.mdが定める文体一貫性を他のLLM生成文言と揃えるため。
- 帰結: (a)(b)(c)は「LLM呼び出し前にアプリケーション側が選択した文脈をシステム
  プロンプトへ追加指示として注入し、その回は通常の受注メモ生成(厳守事項1〜8・7a〜7c)を
  行わず注入文脈に対応するstatusのみを返す」という統一的な扱いとし、llm-system-prompt-
  draft.mdに7a〜7cとは別区分の「文脈注入時の追加指示」として追記する。
- 本フェーズでは3つのうち最も設計が確定している(a)契約者交代確認・期限切れ案内の
  プロンプト文面をllm-system-prompt-draft.mdへ新設した。(b)(c)はプロンプト側で渡す
  情報(確認対象の候補者名/残すメンバー一覧の渡し方)の整理がまだ必要なため次の課題
  として残した。
...
いないためpending-approval.mdへの追記なし。次回は(b)(c)のプロンプト文面新設、その後
