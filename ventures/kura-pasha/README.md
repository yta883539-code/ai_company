...
- LEVOLについて、馬具修理の受注比率(乗馬靴専門か、馬具修理も相応の比重があるか)を追加
  確認し、除外するか候補として残すかを最終判断する。
- エクウスワールド(伊藤政男氏)が修理作業自体を自ら行っているか(除外条件「小売・仲介のみ」
  に該当しないか)を追加確認する。
- 優先順位が固まった候補から、初回コンタクト文面(line-reservation-ai・course-set-pasha
  相当)の草案作成に着手する。実際の連絡・ヒアリング依頼はオーナー承認が必要な範囲として
  別途pending-approval.mdに記録する。
