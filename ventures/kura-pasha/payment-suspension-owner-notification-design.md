...
- ~~payment-failure-dunning-design.md「残課題」がもう1件挙げていた「3日前リマインド送信
  ...
  次回以降はそちらへの統合を優先候補とする。~~ → 記載誤り。3日前リマインド送信専用
  スケジューラ自体は本ドキュメント作成(フェーズ116)より前のフェーズ112時点で
  daily-scheduler-design.md 3.2節・`select_due_payment_failure_reminders()`として
  既に実装済みだった(本ドキュメントの対象は決済失敗3日前の契約者向けリマインドではなく
  制限モード移行時のオーナー向け通知であり、両者は別物)。フェーズ117でこの記載漏れに
  気付き取り消し線で訂正した。代わりに、本ドキュメントの`send_payment_suspension_owner_
  notifications()`自体がdaily-scheduler-design.md 2節のCloud Function G構成図に
  未記載だった欠落をフェーズ117で解消済み(同ドキュメント2節4)参照)。
...
最終更新: 2026-09-14 14:00 UTC(フェーズ117: 「8. 今後の課題」の3日前リマインド
スケジューラに関する記載誤りを訂正〈既にフェーズ112で実装済みだった〉。あわせて
daily-scheduler-design.md 2節のCloud Function G構成図に本モジュールの呼び出し〈4)〉を
追記し、欠落していた統合ポイントの記載を解消)
