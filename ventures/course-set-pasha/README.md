...
- フェーズ133(2026-08-30 19:00 UTC): data-retention-policy.mdの「今後の課題」節に残って
  いた「unfollow-event-handling-design.mdが別途今後の課題として残した『ブロックしたのに
  課金だけ続く』状態へのオーナー向けFAQ整備は、本文書のスコープ外として引き続き未着手」
  という記載が、実際にはフェーズ86・unfollow-billing-faq.mdで既に対応済みだった記載漏れを
  発見・解消した(README「次にやること」1708〜1709行目には正しくフェーズ86対応済みと
  記載されていたが、data-retention-policy.md本体側が未更新のままだった)。コード変更は
  無し(ドキュメント整理のみ)、venture全体419件全件パス・schema検証9件パスを再確認した
...
- フェーズ136(2026-08-31 09:00 UTC): payment-failure-dunning-design.md 4節末尾に残っていた、
  ...
  schema検証9件パスを再確認した。承認不要なドキュメント整理のみで、外部サービスへの
  公開・アカウント作成・支払い等は今回発生していないためpending-approval.mdへの追記なし。
- フェーズ137(2026-08-31 14:00 UTC): trial-end-scheduler-design.md 2節「『有料転換済み
  ユーザーの除外』という残課題への対応方針」の記載を棚卸しした結果、節冒頭・および節内の
  2箇所の注記が、フェーズ102(設計時点)における未解決のギャップをそのまま現在形で
  記述したままで、実際にはフェーズ103(`upgraded_at`書き込み配線)・フェーズ130
  (スケジューラ側読み取り配線・結線テスト)で既に解消済みだった記載漏れ(フェーズ127・
  132・133・136と同種のドキュメント整合性メンテナンス)を発見・解消した。節見出しに
  「(解消済み・フェーズ103/130)」を付記し、冒頭段落を過去形+解消済みの参照に改め、
  節内の2箇所の残存注記にも解消済みである旨の一文を追記した。コード変更は無し
  (ドキュメント整理のみ)、venture全体423件全件パス
  (`python3 -m unittest discover -s prototype -p "test_*.py"`)・
  `schema/validate_test_cases.py`9件全件パスを再確認した。承認不要なドキュメント整理のみで、
  外部サービスへの公開・アカウント作成・支払い等は今回発生していないため
...
- フェーズ140(2026-09-01 22:00 UTC): stripe-webhook-event-dispatch-design.md(フェーズ94)
  ...
  支払い等は今回発生していないためpending-approval.mdへの追記なし。
- フェーズ141(2026-09-02 01:00 UTC): payment-failure-dunning-design.md(フェーズ117作成)
  「背景」節に残っていた記載漏れを発見・解消した。「解約時のデータ削除候補化
  (stripe-cancellation-deletion-candidate-trigger-design.md相当は本venture未着手、別課題)」
  という記述は誤りで、実際にはフェーズ91(2026-08-22)の時点で本venture自身が
  stripe-cancellation-deletion-candidate-trigger-design.mdとして既に設計済みだった
  (フェーズ140で発見・修正した2件の記載漏れと同種、フェーズ117作成時点で既存ドキュメントの
  存在を見落としていたもの)。該当箇所を、フェーズ91で既に設計済みである旨・フェーズ117
  作成時点での誤記であった旨に訂正した。ドキュメント間の整合確認・記載修正のみでコード
...
- フェーズ144(2026-09-02 09:00 UTC): フェーズ143の残課題だった、
  ...
  サービスへの公開・アカウント作成・支払い等は今回発生していないためpending-approval.mdへの
  追記なし。
- フェーズ145(2026-09-02 14:59 UTC): 各設計docの残課題を棚卸ししたところ、
  character-limit-fallback-design.md冒頭の「実装未着手・動作未検証」、および「残課題」節に
  残っていた「`check_message_length_within_line_limit()`の実装・`cloud_function_webhook.py`
  への配線は本フェーズに続けて着手する」という記載が、実際にはドキュメント自体を新規作成した
  フェーズ138(2026-08-31 19:00 UTC)の中で設計と同時に実装・テスト追加まで完了済み
  だったにもかかわらず更新が漏れていた記載漏れ(aircon-pashaのフェーズ167・172等と同種の
  ドキュメント棚卸し)であることを発見・解消した。コード変更は無し(ドキュメント整理のみ)、
  venture全体471件全件パスを再確認した。承認不要なドキュメント整理のみで、外部サービスへの
  公開・アカウント作成・支払い等は今回発生していないためpending-approval.mdへの追記なし。
- フェーズ146(2026-09-02 15:59 UTC): pricing-plan.md「未検証の仮説」に残っていた従量単価
  (100〜150円/回)の妥当性検証(要検証のまま)に対応した。これまでllm-api-cost-estimate.md
  (原価側)のみだった検証に加え、WebSearchで「クラウドソーシング SNS投稿文作成 相場」を
  調査し、人手発注の相場(1件あたりおおむね200円〜2,000円程度)と比較する価値側の検証を
  新規にoverage-price-value-validation.mdとして作成した。本ventureの従量単価は人手発注の
  相場よりも安い水準にあり、原価側・価値側の両面から妥当(むしろ保守的)と判断した。ただし
  演者本人の執筆時間削減という本ventureの提供価値は人手外注とは性質が異なる点、検索結果に
  基づく概算である点は留保として明記した。コード変更は無し(調査・ドキュメント作成のみ)、
...
- フェーズ156(2026-09-04 01:00 UTC): フェーズ155「次回以降の課題」に残っていた
  「解約予約受理時点(`cancel_at_period_end`の`false→true`変化)」「解約取り消し時点
  (`true→false`変化)」の顧客向けLINE通知を実装した。line-reservation-aiフェーズ続き185
  (`customer-subscription-updated-event-routing-design.md`、
  `cloud_function_subscription_cancelled_webhook.py`
  `classify_subscription_update()`・`handle_subscription_updated()`)を翻案し、
  subscription-cancellation-scheduled-notification-design.mdを新規作成した。
  `prototype/subscription_cancellation_notification.py`に
  `classify_cancel_at_period_end_change()`・
  `render_subscription_cancellation_scheduled_message()`(期間終了日・Stripeカスタマー
  ポータルURLを差し込み、`payment_recovery_notification.py`と同じ
  `PORTAL_LINK_PLACEHOLDER`+`PORTAL_LINK_UNAVAILABLE_FALLBACK`方式を踏襲)・
  `render_subscription_cancellation_rescheduled_message()`・
  `handle_subscription_cancellation_update()`を追加した。`stripe_webhook.py`の
  `customer.subscription.updated`分岐(プラン変更検知の直後)に、
  `event.data.previous_attributes.cancel_at_period_end`との前後比較による配線を追加した
  (`previous_attributes`に`cancel_at_period_end`キーが無い場合は変化なしとして扱う)。
  `StripeDispatchResult`に`cancellation_scheduled_notified_user_ids`・
  `cancellation_rescheduled_notified_user_ids`・
  `cancellation_update_notification_failed_user_ids`を新設した。本イベントは契約継続中
  (scheduled)または契約継続確定(rescheduled)時のみ発火するため`store`側の状態変更は
  行わない。本ventureは`suspension_reason`相当の別立て状態を持たないため、
  line-reservation-ai版にある「決済失敗による制限モード中は解約予約通知を出さない」
  ガードは移植せず、検証課題として設計docの「次回以降の課題」に残した(design 3節・7節)。
  テスト20件追加(新規モジュール15件・`test_stripe_webhook.py`5件)、venture全体546件
  全件(`python3 -m unittest discover -p "test_*.py"`)パス・schema検証9件
...
- フェーズ167(2026-09-05 23:00 UTC): 未走査の設計docの残課題棚卸しの一環として
  ...
  今回発生していないためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の
  前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする。
- 最終更新: 2026-09-08 18:00 UTC(フェーズ200まで反映。フェーズ168〜200の追記は本文
  ステータス欄・「次にやること(候補)」節末尾の該当箇所を参照)
...
- (新規解消・フェーズ214、2026-09-14 18:00 UTC: 「次にやること」冒頭のフェーズ204欄が
  ...
  運営していますが、店舗ごとに契約が必要ですか?」を新規追加済みだった記載漏れを発見・
  訂正した。詳細は本節末尾のフェーズ214参照。複数店舗事業者への需要ヒアリング〈customer-
  interview-design.md実施後〉は引き続き次回以降の課題として残る)
- (新規解消・フェーズ204、2026-09-11 18:00 UTC: pricing-plan.mdのセッター複数プラン想定
  顧客像「複数店舗を運営する事業者」が、実装(user_profile/{user_id}単位=1 user_id=1契約)
  では複数の異なるLINEアカウントでの共同利用をサポートしていないギャップを発見し、
  multi-setter-shared-usage-design.mdでMVPスコープを「1つのLINEアカウントを店舗内で共有
  運用する」前提に明確化した。詳細は上記フェーズ204参照。landing-page-copy-draft.mdの
  FAQへの前提補足はフェーズ205で対応済み、複数店舗事業者への需要ヒアリングは次回以降の
  課題として残る)
- (新規解消・フェーズ200、2026-09-08 18:00 UTC: landing-page-copy-draft.md「未検証の仮説・
  要検証事項」の「AI利用の開示表示義務」項目について、フェーズ199で自ら加えた「legal-notices-
  draft.mdにこの論点への言及が無い」という記載が誤りだったと判明した(実際にはフェーズ43時点で
  同ドキュメント2.5節に既に記載済み)ため再訂正した。詳細は上記フェーズ200参照。法律専門家に
  よる最終判断は引き続き未解決のまま残る。次は他venture・アイデア領域の前進、または引き続き
  未走査の設計docの残課題棚卸しを候補とする)
- (新規解消・フェーズ199、2026-09-08 08:00 UTC: landing-page-copy-draft.md「次のステップ
  候補」に残っていた「プライバシーポリシー文面草案…本venture未着手」という古い記載漏れを、
  実際にはフェーズ43でlegal-notices-draft.mdとして対応済みだった内容を反映して訂正した。
  詳細は上記フェーズ199参照。次は他venture・アイデア領域の前進、または引き続き未走査の
  設計docの残課題棚卸しを候補とする)
- (新規解消・フェーズ198、2026-09-07 23:00 UTC: overage-price-value-validation.md
  「残課題」に残っていた一次情報(ランサーズ・クラウドワークス公式相場表)での裏付けを
  WebSearchで実施した。詳細は上記フェーズ198参照。単価自体の変更は無し。次は他venture・
  アイデア領域の前進、または引き続き未走査の設計docの残課題棚卸しを候補とする)
...
- (新規解消・フェーズ156、2026-09-04 01:00 UTC: フェーズ155「次回以降の課題」に残っていた
  ...
  subscription-cancellation-scheduled-notification-design.md参照。決済失敗による制限
  モード中の解約予約通知の扱いは検証課題として残る)
...
- (解消済み 2026-09-02 15:59 UTC・フェーズ146: pricing-plan.md「未検証の仮説」に残っていた
  ...
  上記フェーズ146参照。決済代行サービス側の都度課金対応可否確認は引き続き要検証・オーナー
  承認待ちの範囲として残る)
- (解消済み 2026-09-02 14:59 UTC・フェーズ145: character-limit-fallback-design.mdに残っていた
  「実装未着手」記載漏れ(実際はフェーズ138で実装済み)を解消した。詳細は上記フェーズ145参照)
...
- フェーズ64(2026-08-16 20:58 UTC): 個人経営ボルダリングジムオーナー区分の新規候補探索を、
  ...
  提案する形で記録した。
- フェーズ195(2026-09-06 08:00 UTC): llm-api-cost-estimate.md・unit-economics-estimate.mdが
  提起していた「プロンプトキャッシュ(1時間TTL)導入」の実装方針をprompt-caching-design.mdで
  整理した。claude-apiスキルの最新仕様を確認し、`cache_control: {"type": "ephemeral", "ttl":
  "1h"}`をシステムプロンプト+スキーマ末尾ブロックに付与する実装方法(ベータヘッダー不要)を
  明文化した。あわせて、従来の試算が「1オーナー単独の週2〜3回という利用間隔」を根拠に1時間
  TTLの有効性を結論づけていた点を訂正し、システムプロンプトが全オーナー共通である以上、
...
- フェーズ197(2026-09-07 18:58 UTC): 未走査の設計docの残課題棚卸しの一環として
  ...
  アイデア領域の前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする。
- フェーズ198(2026-09-07 23:00 UTC): overage-price-value-validation.md(フェーズ146)
  「残課題」1点目に残っていた「一次情報(ランサーズ・クラウドワークス公式の相場表)での
  裏付けは未実施」に対応した。WebSearchでランサーズの「SNSライティングの外注・制作依頼」
  公式カテゴリページ(SNS投稿代行は記事文字数×3円〜)、クラウドワークスの公式発注ガイド
  ページ(記事作成は専門知識不要なら1円/字〜、専門性ありなら2〜5円/字、目安2,000円以上)を
  確認し、本ventureの出力想定文字数(100〜300字程度)に当てはめると1件あたり100〜900円と
  なり、現行の従量単価100〜150円/回は公式の相場情報でも「妥当、保守的(安すぎる可能性)側」
  という従来評価の裏付けが取れることを確認した。結論・単価自体は変更なし。コード変更は無く、
  venture全体573件全件(`python3 -m unittest discover -s prototype -p "test_*.py"`)・
  schema検証9件(`python3 schema/validate_test_cases.py`)パスを確認した(変更前と同じ結果)。
  承認不要なWebSearch調査・設計doc記載の追記のみで、外部サービスへの公開・アカウント作成・
  支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。
- フェーズ199(2026-09-08 08:00 UTC): 未走査の設計docの残課題棚卸しの一環として
  landing-page-copy-draft.md(フェーズ42)「次のステップ候補」1点目に残っていた
  「特定商取引法に基づく表記・プライバシーポリシーの文面草案の作成…本venture未着手」という
  記載が、実際にはフェーズ43でlegal-notices-draft.mdとして既に作成済み(特商法表記・
  プライバシーポリシーの両方を含む)であるにもかかわらず更新されずに古い「未着手」表記の
  まま残っていた記載漏れだと判明した(フェーズ167・197等と同じ「対応済みなのに参照元の
  記載が更新されないまま残る」パターンの再発)。該当bulletを取り消し線付きで解消済みに
  更新し、対応内容(legal-notices-draft.md)への参照を追記した。あわせて同ドキュメント
  「未検証の仮説・要検証事項」の「AI利用の開示表示義務」項目にあった同種の古い記載
  (「本ventureはまだlegal-notices-draft.mdに相当する文書を作成していない」)も、
  文書自体は存在するがこの論点への言及が無く未解決のまま残っている旨に訂正した。コード
  変更は無く、venture全体573件全件(`python3 -m unittest discover -s prototype -p
  "test_*.py"`)・schema検証9件(`python3 schema/validate_test_cases.py`)パスを確認した
  ...
  優先候補とする。
- フェーズ200(2026-09-08 18:00 UTC): 未走査の設計docの残課題棚卸しの一環として
  landing-page-copy-draft.md「未検証の仮説・要検証事項」のうち、フェーズ199で自ら
  追記した「AI利用の開示表示義務」項目の記載(「本venture向けのlegal-notices-draft.mdは
  フェーズ43で作成済みだが、この論点自体への言及は同ドキュメントにも無く、未解決のまま
  残っている」)自体に誤りがあることを発見した。legal-notices-draft.md 2.5節を確認した
  ところ、実際にはフェーズ43(legal-notices-draft.md新規作成)の時点で既に「AIが下書き
  生成に関与していることの開示要否」として、line-reservation-aiのweb調査結果(日本法上は
  一般的な義務なし、ステマ規制・EU AI法は直接適用対象になりにくい)を踏まえつつ、本venture
  固有のSNS投稿文(対外発信)特有のステルスマーケティング規制論点を独自に追加する形で
  明記されていた(フェーズ43本文にも「ステルスマーケティング規制との関係を独自論点として
  追加」と明記されている)。つまりフェーズ199は「対応済みなのに参照元の記載が古いまま
  更新されていない」パターン(フェーズ167・196・197・199自身の1点目等)の再発を疑って
  調査した結果、実際には論点自体は最初から反映済みだったにもかかわらず、確認不足により
  逆に「言及が無い」という誤った訂正を加えてしまっていたと判明した。該当箇所を、
  legal-notices-draft.md 2.5節に実際に記載されている内容(ステマ規制との関係の独自
  再整理を含む)を正しく反映する形に再訂正した。法律専門家による最終判断が未了である点
  自体は変わらず未解決のまま残る。コード変更は無く、venture全体573件全件
  (`python3 -m unittest discover -s prototype -p "test_*.py"`)・schema検証9件
  (`python3 schema/validate_test_cases.py`)パスを確認した(変更前と同じ結果)。承認不要な
  設計doc記載の整合性修正のみで、外部サービスへの公開・アカウント作成・支払い・送信等は
  今回発生していないためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の
  前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする(なお、この種の
  「記載の食い違いを疑って調査したが実際には食い違いが無かった」誤り自体の再発防止のため、
  今後は参照先ドキュメントの該当節を実際に開いて内容を確認してから訂正を加える、という
  手順の徹底を教訓として残す)。
- フェーズ201(2026-09-09 11:00 UTC): 未走査の設計docの残課題棚卸しの一環として
  checkout-initiation-flow-design.md(フェーズ98)「残課題」を確認したところ、
  「トライアル終了通知メッセージ自体(1(a))は本venture未設計。次の課題として残す」という
  記載が残ったままになっていた。しかしtrial-end-notification-design.md(フェーズ99)を
  実際に開いて確認したところ、この課題はフェーズ99で通知メッセージ本文・トリガー条件
  (生成回数5回到達/期間14日到達のいずれか早い方)として既に設計され、その後フェーズ100
  (トライアル起点確定)・フェーズ105(trial_generation_count実装)・フェーズ106(所要時間
  試算配線)・フェーズ114(`_is_generation_paused()`による生成一時停止実装)まで一貫して
  対応済みであることを確認した(フェーズ200の教訓を踏まえ、参照先を実際に開いて内容を
  確認してから訂正した)。実LIFF登録・実Cloud Scheduler実行環境の構築のみオーナー承認待ちの
  範囲として引き続き残ることを明記し、checkout-initiation-flow-design.mdの該当bulletを
  解消済みに更新した。コード変更は無く、venture全体573件全件(`python3 -m unittest discover
  -s prototype -p "test_*.py"`)・schema検証9件(`python3 schema/validate_test_cases.py`)
  いずれもパスを確認した(変更前と同じ結果)。承認不要な設計doc記載の整合性修正のみで、
  外部サービスへの公開・アカウント作成・支払い・送信等は今回発生していないため
...
- フェーズ203(2026-09-11 14:00 UTC): 他venture
  ...
  支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。
- フェーズ204(2026-09-11 18:00 UTC): pricing-plan.mdのセッター複数プラン想定顧客像
  「複数ジムを掛け持ちするフリーランスセッター、複数店舗を運営する事業者」を精査したところ、
  前者(1人のセッターが複数ジムを掛け持ち)はline-user-id-linking-design.mdのgym_area_pairs
  設計で既に実現済みだが、後者(複数店舗を運営する事業者)は文言上「複数の異なるスタッフが
  それぞれ自分のLINEアカウントから送信し1契約の利用枠を共有する」ケースを連想させる一方、
  実装は`user_profile/{user_id}`単位(1 user_id = 1契約)が前提で、複数の異なるuser_idが
  1契約を共有する仕組みが存在しない未対応のギャップを発見した。multi-setter-shared-usage-
  design.mdとして検討した結果、本ventureは単方向バッチ処理で送信者個人を区別する業務要件が
  無く、kura-pasha型のworkshop・契約者概念を横展開するのは実需要未確認の段階ではオーバー
  エンジニアリングと判断し、MVPでは「複数店舗を運営する事業者」を「1つのLINEアカウントを
  店舗内で複数スタッフが共有運用する」前提と明確化するスコープ決定を行った(コード変更は
  不要)。pricing-plan.mdの想定顧客像欄にこの前提を追記する訂正を行った。venture全体575件
  全件・schema検証9件いずれもパス(変更前と同じ結果)を確認した。承認不要な設計文書作成・
  既存ドキュメントの記載明確化のみで、外部サービスへの公開・アカウント作成・支払い・送信等は
  今回発生していないためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の
...
- フェーズ213(2026-09-14 12:00 UTC): candidate-longlist-draft.md「次回以降の課題」に
  2026-08-08 04:00 UTCの第五弾から残っていた「フリーランスセッター区分候補の探索は未着手
  のまま持ち越し」という記載漏れを発見・訂正した。実際には第二弾(通常探索)・第五弾
  (逆引きアプローチ)の計3ラウンドで既にWebSearch探索済みであり、3回連続で新規候補0件
  だったことを受けて第五弾時点で「当面保留、個人経営ジムオーナー区分を優先」という方針が
  確定していた(README.mdフェーズ18で反映済み)。フェーズ208・210・211・212と同種の
  cross-document記載整合性修正パターンだが、今回は候補探索ドキュメント自身の「次回以降の
  課題」節(他ドキュメントから参照されるだけで自身は更新されないまま古い記載が残っていた)
  が対象だった。コード変更は無く、回帰確認としてventure全体583件(`python3 -m unittest
  discover -s prototype -p "test_*.py"`)・schema検証17件(`python3 schema/validate_
  test_cases.py`)いずれもパス(変更前と同じ結果)を確認した。承認不要なドキュメント
  整合性修正のみで、外部サービスへの公開・アカウント作成・支払い・送信等は今回発生して
  いないためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の前進を
  優先候補とする。
- フェーズ214(2026-09-14 18:00 UTC): 「次にやること(候補)」節冒頭のフェーズ204欄に
  「landing-page-copy-draft.mdのFAQへの前提補足…は次回以降の課題として残る」という
  記載が残ったままだったが、実際にはフェーズ205(2026-09-11 22:00 UTC)でFAQ「複数店舗を
  運営していますが、店舗ごとに契約が必要ですか?」を新規追加し既に解消済みだった記載漏れを
  発見・訂正した。フェーズ208・210・211・212・213と同種のcross-document記載整合性修正
  パターンだが、今回は「次にやること」節そのもの(フェーズ進行ログ本体とは別に一覧として
  参照されるだけで、後続フェーズで前提が解消されても自身は更新されないまま古い記載が
  残っていた)が対象だった。フェーズ204欄の該当記載を「FAQへの前提補足はフェーズ205で
  対応済み」に更新し、節冒頭にフェーズ214の解消記録を追加した。複数店舗事業者への実際の
  需要ヒアリング(customer-interview-design.md実施後の課題)は引き続き未解決のまま残る。
...
- 最終更新: 2026-09-19 14:00 UTC(フェーズ226: history_rows空配列禁止ルールを検証する
  ...
  フィクスチャ・ドキュメント更新のみ)
- フェーズ227(2026-09-19 21:00 UTC定例更新): ci-setup.mdの棚卸しを行い、「実施内容」
  「追記(2026-08-08 22:00 UTC)」節がCI導入当初(フェーズ29)のテスト件数(prototype 18件・
  schema検証6件)のまま更新されずに残っていた記載漏れを発見した。
  `.github/workflows/course-set-pasha-tests.yml`自体は`unittest discover`・
  `validate_test_cases.py`を呼び出すのみで新規テストファイルも自動的に対象へ含まれる
  構成のため、ワークフロー自体の修正は不要と確認した。ローカルで
  `python3 -m unittest discover -p "test_*.py" -v`(prototype/)・
  `python3 validate_test_cases.py`(schema/)を実行し、prototype全605件・schema検証21件
  いずれもパス(既存ロジックへの変更は無く件数の実態確認のみ)であることを確認し、
  ci-setup.mdに追記した。あわせて、今後は同ファイル本体の件数を都度更新する運用とはせず
  最新件数はREADME.mdの最終更新記載を参照する方針である旨も明記した。判定ロジック・
  実装への変更は無い。承認不要なドキュメント記載訂正のみで、外部サービスへの公開・
  アカウント作成・支払い・送信等は今回発生していないためpending-approval.mdへの追記
  なし。次回は他venture・アイデア領域の前進、または引き続き未走査の設計docの残課題
  棚卸しを優先候補とする。
...
- フェーズ233(2026-09-20 21:00 UTC定例更新): support-cost-estimate.md(フェーズ216)
  「残課題」に「サポート対応の外部委託・セルフサービス化(FAQ整備等)によるコスト削減余地は
  本フェーズでは検討していない」として未着手のまま残っていた論点に着手した。既存の
  landing-page-copy-draft.mdのFAQ・onboarding-guide.md・unfollow-billing-faq.mdを棚卸し
  したところ、support-cost-estimate.mdのオンボーディング工数試算(15〜30分/件)のうち
  「料金プラン・トライアル条件の説明」分(5〜10分)は既存資産で大部分カバー可能なはずが、
  そのまま丸ごと工数計上されていた齟齬を発見した。support-cost-selfservice-reduction.mdを
  新規作成し、既存資産の整備を前提にオンボーディング工数は15〜30分→10〜20分程度(約1/3
  削減)、継続対応工数は5〜20分→4〜16分程度(2割弱削減)、対応可能顧客数の目安は約120顧客→
  約140〜150顧客程度に緩やかに拡大しうるとの机上試算を行った(いずれも実測データのない
  仮定であり、実運用データでの再検証が必須)。support-cost-estimate.md「残課題」に本
  ドキュメントへの参照を追記した。コード変更は無く、回帰確認としてventure全体605件
  ...
  支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。チャットボット
  による一次受付自動化の検討、他venture固有のFAQ・案内資産の棚卸しは次回以降の課題として
  残す。
- 最終更新: 2026-09-20 21:00 UTC(フェーズ233: support-cost-estimate.mdの残課題だった
  セルフサービス化コスト削減余地の検討にsupport-cost-selfservice-reduction.mdとして着手し、
  既存FAQ・案内資産の棚卸しに基づく粗い削減余地〈オンボーディング約1/3・継続対応2割弱〉を
  試算。コード変更は無し)
- フェーズ234(2026-09-21 01:00 UTC定例更新): フェーズ233(support-cost-selfservice-
  reduction.md)の残課題だった「チャットボットによる一次受付の自動化(FAQ該当質問への
  自動応答)は本フェーズでは検討していない(次回以降の課題)」に着手した。
  chatbot-first-response-feasibility.mdを新規作成し、既存のtech-stack.md構成(LINE公式
  アカウント⇄GCP Cloud Functions⇄LLM API)を前提に、(1)キーワードマッチ方式、
  (2)LLMによる意図分類+定型回答方式、(3)LLMによる自由回答方式、の3つの技術構成選択肢を
  比較した。表記揺れへの弱さ(方式1)、ハルシネーションによる誤回答リスク(方式3)を
  踏まえ、FAQ原文をそのまま返す方式2(意図分類+定型回答)が主要顧客層(LINE操作に不慣れな
  場合がある個人事業主)向けの第一候補として現実的と結論づけた。あわせて、意図分類を誤って
  本来の投稿文生成依頼をFAQ回答に振り分けてしまうリスク(本サービスの中核機能への悪影響)、
  エスカレーション導線(人的対応が必要な場合の運営者通知方法)が未設計であることをリスク・
  残課題として整理した。実装・実LLM呼び出し・実LINE接続は行わない机上の技術構成検討に
  とどめた。コード変更は無く、回帰確認としてventure全体605件(`python3 -m unittest
  discover -s prototype -p "test_*.py"`)・schema検証21件(`python3 schema/
  validate_test_cases.py`)いずれもパス(変更前と同じ結果)を確認した。承認不要な設計
  文書作成のみで、外部サービスへの公開・アカウント作成・支払い・送信等は今回発生して
  いないためpending-approval.mdへの追記なし。次回は意図分類プロンプトの具体設計・
  エスカレーション導線の設計、または他venture・アイデア領域の前進を優先候補とする。
- 最終更新: 2026-09-21 01:00 UTC(フェーズ234: チャットボットによる一次受付自動化の技術
...
- フェーズ248(2026-09-23 04:00 UTC定例更新): chatbot-first-response-feasibility.md
  (フェーズ234作成)「残課題」に残っていた、他venture(aircon-pasha・kura-pasha・
  line-reservation-ai)へのチャットボット一次受付自動化検討の横展開が未着手のまま
  古い記載として残っていた点に対応した。aircon-pasha・kura-pashaのtech-stack.mdを
  確認し、両venture共に本ventureと同一の「LINE公式アカウント⇄Webhookサーバー⇄
  LLM(3出力生成)⇄返信メッセージ」という単方向バッチ処理構成であることを確認、
  本ドキュメントの一次受付自動化の設計思想(対象範囲の切り分け・意図分類+定型回答
  方式)は両venture固有のFAQ内容に置き換えるだけで横展開できる可能性が高いと判断した
  (実際の設計移植・実装は各venture側の次回以降のフェーズとして申し送り、本フェーズ
  では着手せず)。line-reservation-aiは既にLLMによる会話応答・意図判定が中核機能
  として存在し双方向の会話状態管理を前提とする設計のため、本ドキュメントが想定する
  「単方向バッチ処理への一次受付分岐追加」という構成自体が当てはまらないことを確認し、
  同venture固有の検討は別軸の課題として申し送った。あわせて、フェーズ235
  (意図分類プロンプト・エスカレーション導線設計)、フェーズ244〜247
  (webhookエントリポイントまでの実装完了)によって既に解消済みだった「意図分類
  プロンプト設計は次回以降の課題」「エスカレーション導線設計は未着手」という2項目が
  本ドキュメントの「残課題」節で古い記載のまま更新されずに残っていた記載漏れを発見し、
  解消済みである旨と対応フェーズへの参照を追記して訂正した。コード変更は無く、
  回帰確認として本venture全体648件(`python3 -m unittest discover -s prototype -p
  "test_*.py"`)・schema検証21件(`python3 schema/validate_test_cases.py`)いずれも
  パス(変更前と同じ結果)を確認した。承認不要な既存ドキュメントの監査・訂正のみで、
  外部サービスへの公開・アカウント作成・支払い・送信等は今回発生していないため
  pending-approval.mdへの追記なし。次回候補: aircon-pasha・kura-pasha側での
  チャットボット一次受付自動化設計の実移植着手、line-reservation-ai向けの別軸検討
  (既存の意図判定へのFAQ系インテント追加)、または他venture・アイデア領域の前進。
- 最終更新: 2026-09-23 04:00 UTC(フェーズ248: chatbot-first-response-feasibility.mdの
  「残課題」に残っていた他venture横展開の検討を実施。aircon-pasha・kura-pashaは
  同一アーキテクチャのため横展開可能性が高いと判断(実移植は各venture側へ申し送り)、
  line-reservation-aiは会話状態管理を前提とする別構成のため対象外と判断。あわせて
