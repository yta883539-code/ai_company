...
- history-export-usage-guide.mdの手順はGoogle スプレッドシートのブラウザ版を想定した
  未検証の手順であり、オーナーが実際に使うツール・環境(モバイルアプリ版/Excel等)が
  判明した段階で見直しが必要。
- post_generation_checks.pyは文単位判定への変更で別エリア混在時の見逃しを解消したが、
  読点区切りで1文にまとまる同様のケース(post-generation-checks-cross-area-review.md
  「残る既知の限界」参照)は未対応。実LLM接続後の実際の生成文での発生頻度確認も必要。
