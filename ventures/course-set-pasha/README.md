...
- フリーランスセッター区分は、個人経営ジムのSNS投稿から「ゲストセッター」「セット協力」等の
  タグ付け人物を辿る逆引きアプローチで再探索する(第二弾からの申し送り、未着手)。
- 複合ジムオーナー区分は登攀道場が候補から外れたため、他の個人経営複合ジム候補の再探索が
  必要(検索キーワードの見直しを含む)。
- 候補2(AT WALL)は3店舗展開の系列であることが判明したため、運営体制(1店舗あたりの
  スタッフ数・課題入れ替えの意思決定者が店舗ごとにいるか)を追加のSNS投稿閲覧で確認する。
- ロングリストが揃った段階で、質問数14問が想定時間(10〜15分)に収まるかのリハーサル台本を
  作成し机上検証する。
