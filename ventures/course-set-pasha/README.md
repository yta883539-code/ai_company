...
- candidate-longlist-draft.mdの「次回以降の課題」にある通り、登攀道場のリード壁有無を確認し
  複合ジム区分の候補として確定できるか判断する。
- フリーランスセッター区分は、個人経営ジムのSNS投稿から「ゲストセッター」「セット協力」等の
  タグ付け人物を辿る逆引きアプローチで再探索する。
- 候補1(FRICTION FREAKS)・候補2(AT WALL)について、除外条件・必須条件への該当有無を
  SNS投稿の個別閲覧で確認する。
