...
(2026-09-02追記・フェーズ142: 本節は`gym_area_pairs`・`email`等の「ユーザー設定・利用実績
データ」を対象にした結論であり、blocked-but-billing-detection-design.mdで新設した
`is_following`フラグはこの結論の対象外(「ブロック中かつ契約継続中」検知専用の別種の状態)
として、follow/unfollowイベント受信のたびに更新する。詳細は同ファイル参照。)
