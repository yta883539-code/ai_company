...
## 2. 「有料転換済みユーザーの除外」という残課題への対応方針(解消済み・フェーズ103/130)

条件(B)の対象は「トライアル中で、かつ期間が到達したユーザー」に限定する必要がある。
設計時点(フェーズ102)では、course-set-pasha prototypeに「有料転換済みかどうか」を
判定するフィールドがusage_counter・他のいずれのストアにも存在せず(stripe_webhook.pyは
Checkout Session完了イベントの受信・ディスパッチまでは実装済みだったが、受信後に
usage_counter側へ「有料転換済み」を書き込む処理が本ventureで未実装だった)、以下は
その時点でのギャップに対する対応方針の記録である。フェーズ103で書き込み配線、
フェーズ130で選定ロジック側への読み取り配線(結線テストによる実際の接続確認込み)が
完了しており、本節が示す「残課題」は現在は解消済み(詳細は本節末尾の解消済み注記参照)。
...
- 選定ロジック(3節)は`upgraded_at is not None`のユーザーを対象から除外する形で
  ...
  →(解消済み・同フェーズ103: 上記の通り`handle_checkout_session_completed()`への
  書き込み配線を同フェーズ内で完了した。)
- 万一`upgraded_at`書き込み配線が未接続のまま条件(B)が先に稼働した場合の安全策として、
  ...
  →(解消済み・フェーズ103/130: 書き込み配線(フェーズ103)・スケジューラ側読み取り配線
  (フェーズ130)が完了したため、この誤送信リスクは既知の限界として残っていない。)
