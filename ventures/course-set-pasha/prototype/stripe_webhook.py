"""Stripe Webhookの署名検証(stripe-webhook-signature-verification-design.md フェーズ93)。

実Stripeアカウント接続(オーナー承認待ち)なしでも検証できる、`Stripe-Signature`ヘッダの
検証ロジックのみを切り出したモジュール。`cloud_function_webhook.py`(LINE側)とは独立した
別ファイルとし、既存のLINE側コードには一切影響を与えない。
"""
