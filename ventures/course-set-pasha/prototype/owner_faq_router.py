...
def _normalize_command_text(text: str) -> str:
    ...
    日本語入力の携帯端末ではIMEが全角モードを既定にしていることが多く、
    「ＦＡＱ」「Ｑ１」のように英数字が全角で入力される場合がある
    (フェーズ221で発見・対応。それまでは半角入力しか想定していなかった)。
    """
...
def is_owner_faq_menu_trigger(text: str) -> bool:
    ...
    大文字小文字を区別せず、前後の空白は無視する。全角英字で入力された場合も
    NFKC正規化により半角相当として一致する(フェーズ221)。
    """
...
def match_owner_faq_item_code(text: str) -> Optional[str]:
    ...
    コード(例: "Q1")を返す。一致しなければNone。大文字小文字は区別せず、
    全角英数字(例: 「Ｑ１」)で入力された場合もNFKC正規化により一致する(フェーズ221)。
    """
