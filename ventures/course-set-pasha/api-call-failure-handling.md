...
1. **Push APIではなくReply APIのみを使う**(tech-stack.md「想定コンポーネント1」)。
   Reply APIは`replyToken`が1回限り・Webhook受信から約1分以内(2019年頃の調査記事では
   約30秒だったが、その後1分程度まで延長されたとする複数の二次情報〈Qiita等の検証記事、
   2026-08-17 WebSearch調査、LINE公式ドキュメントの一次情報はWebFetchのegress制約により
   本エージェントからは引き続き取得不可〉あり)で失効する目安の値として扱う。line-reservation-aiは
...
## 未検証・要検討事項
- Reply APIのトークン有効期限は、2026-08-17のWebSearch調査により「Webhook受信から
  約1分以内・1回限りで、失効後または使用済み後は`Invalid reply token`エラーとなる」旨の
  二次情報(Qiita等の検証記事)を複数確認できた。この値は「即時1回のみリトライ」という
  既存方針(方針2)の妥当性を裏付ける材料になる(1分の猶予内であれば即時リトライは
  間に合う可能性が高い)。ただし失敗レスポンス時にトークン自体が消費済み扱いになるか
  (=リトライで再利用できるか)は二次情報でも確認できておらず、LINE公式ドキュメントの
  一次情報での確認が引き続き必要(line-image-content-api-review.mdと同様、
  WebFetchのegress制約により本エージェントからは直接確認不可と再確認した)。
  実LINE接続後に必ず一次情報で再確認する。
...
## 次のステップ候補
- (一部解消 2026-08-17 10:00 UTC: 「未検証・要検討事項」に残っていたReply APIトークンの
  有効期限について、WebSearchによる二次情報調査を行い「約1分以内・1回限り」という
  目安値の裏付けを得た(詳細は上記「未検証・要検討事項」参照)。LINE公式ドキュメントの
  一次情報でのトークン失効時の消費有無の確認は、WebFetchのegress制約により本エージェント
  からは引き続き取得できず、実LINE接続後の課題として残る)
