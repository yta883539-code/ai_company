...
合否判定する。post_generation_checks.py相当の機械チェックが未実装のため、まずは
schema/validate_test_cases.pyのvalidate_cross_field_rules()で機械チェック可能なものと、
...
| 2 | 厳守事項2(「変更なし」エリアへの誤言及禁止) | unchanged_areasに列挙されたエリアについて、新着課題があるかのような表現が本文に混入していないか | 機械チェック(`unchanged_areas`と本文の突き合わせ、aircon-pashaのpost_generation_checks.py相当のヒューリスティックを新規実装する必要あり)+人手 | G2(変更なしエリア2件を明記) |
...
| 9 | 厳守事項9(ですます調・絵文字は出力1のみ1〜2個まで) | 出力2・出力3に絵文字が含まれていないか、出力1の絵文字が2個を超えていないか、文体が統一されているか | 機械チェック(aircon-pasha post_generation_checks.pyの絵文字パターン検出を流用・出力別に上限を分ける改修が必要) | 全件 |

## aircon-pashaとの差分(移植時の注意点)

- course-set-pashaにはaircon-pashaのpost_generation_checks.pyに相当する機械チェックスクリプトが
  まだ存在しない。上表の「機械チェック」欄はvalidate_test_cases.pyのvalidate_cross_field_rules()
  で対応可能な範囲に限られ、`unchanged_areas`本文突き合わせ・絵文字パターン検出は、実装着手時に
  aircon-pashaのpost_generation_checks.pyを土台に移植する必要がある(現時点では未着手の課題として
  明記するに留める)。
- course-set-pashaは出力1のみ絵文字1〜2個を許容する点がaircon-pasha(全出力で絵文字不使用)と
  異なるため、機械チェックは出力ごとに上限を分けて実装する必要がある。
...
- unchanged_areas本文突き合わせ・出力別絵文字上限チェックの機械化スクリプトは未実装(上記
  「差分」節参照)。実装着手時にaircon-pashaのpost_generation_checks.pyを移植・改修する。
