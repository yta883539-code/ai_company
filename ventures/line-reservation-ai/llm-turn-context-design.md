...
- 案1(会話履歴をLLMに渡す設計)を将来的に採用する場合は、firestore-data-model.mdの
  ...
  試算の見直しが必要になる。今回はスコープ外とした。
  (解消済み 2026-09-11 21:00 UTC: 採用した案2(現状の設計)を前提としたコスト試算は
  llm-api-cost-estimate.mdとして新規作成した。1ターンあたりの入力トークン数が
  会話が進んでも増加しない設計のため、コストは各ターンほぼ一定という試算結果になった。
  案1採用時に必要となる「トークン増加によるコスト試算の見直し」自体は、案1採用が
  決まった時点の課題として引き続き未着手)
