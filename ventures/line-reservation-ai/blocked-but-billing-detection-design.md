...
- ~~候補一覧を実際にオーナーへ届ける手段(代替チャネル、メール等)の設計・実装。
  ...
  (解消済み: blocked-but-billing-owner-email-notification-design.md〈フェーズ続き177〉で
  メール本文組み立て・送信対象抽出・`send_blocked_but_billing_owner_email_notifications()`
  まで設計・実装済み。フェーズ続き178でフォロー再開・解約確定時のクリア配線も完了。
  実際のメール配信基盤〈SendGrid・Gmail API等〉への接続・送信実行自体は同ファイル5節の
  通りオーナー承認待ちの範囲として残る。2026-09-15追記)
