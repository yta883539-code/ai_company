...
- (解消済み 2026-08-03 02:00 UTC: 永続ストレージ側の予約履歴・会話履歴・通知ログの
  保存期間方針をdata-retention-policy.mdで新規設計した。予約実績(confirmed)は3年、
  アーカイブ済み会話状態は`archivedAt`から1年、通知ログは6か月を暫定値として設定。
  実際の削除バッチ実装はホスティング基盤確定・オーナー承認後の課題として残置)
