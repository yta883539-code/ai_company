...
- 「[今週分をCSVで書き出す]」ボタン押下→実際のファイルダウンロードへの配線、および
  ...
  引き続き残る。
- (解消済み 2026-08-10 01:00 UTC: 予約一覧ページの「来店済み」チェック・無断キャンセル確定
  操作から`record_visited()`/`record_no_show_confirmed()`への実呼び出し配線のうち、実際の
  画面描画・クリックイベント自体を除いたロジック部分は`mark_booking_visited()`/
  `mark_booking_no_show_confirmed()`(prototype/engine.py)として実装した。`BookingListEntry`に
  識別用の`store_id`/`slot_key`(CSV非出力)を追加し、`list_booking_entries()`の返り値をそのまま
  渡せるようにした。README.mdフェーズ(続き94)参照。残るのは実際のチェックボックス描画・
  クリックイベントの配線自体で、引き続きホスティング基盤確定後の課題)
