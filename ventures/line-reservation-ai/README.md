...
- conversation-samples-test-cases.md: LLMシステムプロンプト草案・JSONリトライ設計を検証するための会話サンプル(正常系4件・崩れ系16件)のテストケース設計(2026-08-01 17:00 UTC更新、N3(予約確定メッセージ)にフォーマル/standard/カジュアルの3トーン別期待自然文サンプルを追加。JSON側はトーンに関わらず不変であることを明記。前日リマインド等の他テンプレートのトーン別サンプルは未追加。机上設計のみで実LLM検証は未着手)
...
- (一部解消 2026-08-01 17:00 UTC: conversation-samples-test-cases.mdのN3(予約確定メッセージ)に
  フォーマル/standard/カジュアルの3トーン別期待出力サンプルを追加した。前日リマインド・仮押さえ直後・
  FAQ回答テンプレートについても同様のサンプル追加が残課題)
- 前日リマインド・仮押さえ直後・FAQ回答テンプレートについても、N3と同様の形式でトーン別
  (フォーマル/カジュアル)期待出力サンプルをconversation-samples-test-cases.mdに追加する
