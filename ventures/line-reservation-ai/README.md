...
- フェーズ(続き72、2026-08-08 21:00 UTC): 「次にやること」の未確定事項はオーナーの回答待ちで
  動かせないため、reminder-timing-and-resend-rules.mdで設計済みのまま実装が抜けていた箇所が
  ないか点検した。reminder_scheduler.select_due_resends()(当日朝の再送要否判定)は既に実装
  済みだったが、判定後に送るメッセージ本文を組み立てるformat_reminder_resend_message()が
  engine.pyに存在しないままだったことを発見し、新規実装した(_render_by_tone()経由でformal/
  standard/casualの3トーンに対応)。あわせて、同ファイルの「日時・メニューの復唱は省略する」
  という記載が、直下の文言例(時刻・メニュー名を含む)と矛盾していたのを発見し、文言例に合わせて
  「日付のみ『本日』に置き換え、時刻・メニューは再掲する」と訂正した。これまでテストが無かった
  format_reminder_message()と合わせてReminderMessageTestを新規追加(テスト4件追加・全140件パス)。
  実際のLINE Push Message API送信・Cloud Schedulerでの起動は引き続きオーナー承認待ち
  (pending-approval.md参照)で、本項は判断・整形ロジックの実装のみにとどまる。
- 最終更新: 2026-08-08 21:00 UTC
...
- (解消済み 2026-08-08 21:00 UTC: オーナー回答待ちの間、承認不要で前進できる設計・実装の
  抜け漏れがないか点検し、reminder-timing-and-resend-rules.mdで設計済みだったformat_reminder_
  resend_message()の未実装を解消した。詳細はフェーズ(続き72)参照)
- 上記と同種の観点(mdファイルで設計済みだがengine.py/prototype配下に実装が反映されていない
  箇所がないか)の点検を、owner-settings-wireframe.md等の他ドキュメントにも広げる余地がある。
