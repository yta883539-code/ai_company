...
- escalation-consolidation-logic.md: 連続エスカレーション(同一顧客が短時間に複数回)を1通に集約する通知ロジックの設計(2026-07-31時点。時間窓5分固定、初回は即時個別通知+ウィンドウ内の追加分はまとめ通知の2段階方式。件数3件以上は優先確認を促す一文を追加。実装方式は概念メモのみで技術選定は未着手)
...
- escalation-consolidation-logic.mdで未検討とした「医療相談等の緊急度が高い種別を例外的に都度即時通知すべきか」の要否検討
- 集約ウィンドウが連続再発火し続けた場合の上限回数・別手段連絡への切り替えルールの検討
- 通知ログ集計画面で使う「未実装機能」分類ラベルをLLMがどう自動付与するか(プロンプト設計・分類精度の検証方針)の検討
