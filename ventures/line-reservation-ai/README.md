...
- llm-system-prompt-draft.md: 会話フロー・二重予約防止・トーン&マナー・保留タイムアウト・無断キャンセル対応等の既存設計を統合した、LLM会話エンジン向けシステムプロンプト草案(2026-07-30 20:58 UTC更新、厳守事項9aの説明文にowner-settings-wireframe.mdの店舗FAQ情報欄の具体項目[駐車場台数・支払い方法内訳]と未入力時の6番エスカレーションを反映。実装・動作検証は未着手)
...
- conversation-samples-test-cases.md: LLMシステムプロンプト草案・JSONリトライ設計を検証するための会話サンプル(正常系4件・崩れ系9件)のテストケース設計(2026-07-30 17:58 UTC更新、E6・E9で指摘した抜け漏れはllm-system-prompt-draft.mdへ反映済み。机上設計のみで実LLM検証は未着手)
- faq-escalation-boundary.md: 厳守事項9(FAQ/雑談)と6(予約以外の相談エスカレーション)の境界線整理(2026-07-30 19:58 UTC更新、営業時間等の店舗登録済み静的情報は9a、未登録・個別判断が必要な相談は6番に振り分け。owner-settings-wireframe.md側のFAQ入力欄の有無を確認し追加済み。llm-system-prompt-draft.mdの厳守事項9a説明文への反映は未着手)
...
- conversation-samples-test-cases.mdに、9a(店舗FAQ情報が未入力/一部項目のみ入力のケース)・6番エスカレーションの新しいテストケースを追加し、今回の厳守事項9a改訂と整合しているか検証する
