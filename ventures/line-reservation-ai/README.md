...
- faq-escalation-boundary.md: 厳守事項9(FAQ/雑談)と6(予約以外の相談エスカレーション)の境界線整理(2026-07-30 18:58 UTC時点、営業時間等の店舗登録済み静的情報は9a、未登録・個別判断が必要な相談は6番に振り分け。llm-system-prompt-draft.mdへ反映済み、owner-settings-wireframe.md側のFAQ入力欄の有無は未確認)
...
- owner-settings-wireframe.mdに9a(FAQ静的情報)用の入力欄(営業時間・アクセス・駐車場・支払い方法等)が存在するか確認し、なければ追記する
