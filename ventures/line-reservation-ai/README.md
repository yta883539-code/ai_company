...
- llm-system-prompt-draft.md: 会話フロー・二重予約防止・トーン&マナー・保留タイムアウト・無断キャンセル対応等の既存設計を統合した、LLM会話エンジン向けシステムプロンプト草案(2026-07-30時点、実装・動作検証は未着手)
...
- conversation-samples-test-cases.md: LLMシステムプロンプト草案・JSONリトライ設計を検証するための会話サンプル(正常系4件・崩れ系9件)のテストケース設計(2026-07-30時点、机上設計のみで実LLM検証は未着手。雑談・スパム入力と未実装機能問い合わせへの対応がプロンプト未定義であることを指摘)
...
- conversation-samples-test-cases.mdで指摘したE6(雑談・スパム入力)・E9(未実装機能問い合わせ)への対応方針をllm-system-prompt-draft.mdに反映
