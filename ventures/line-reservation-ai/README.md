...
- 上記「ヒアリング依頼提示パッケージ」(interview-request-package.md)で整理した未確定事項
  (謝礼有無・送信者名表記・返信先連絡先・優先度B4件の依頼可否)についてオーナーの回答を待つ。
  回答が得られるまでは、実在店舗・個人事業主への実際の連絡・送信は行わない。
- 余力があれば、美容室・パーソナルジムの代替候補(第三候補)の探索を検討する
  (優先度A6件に美容室・パーソナルジムが1件も入っていないため、業種の偏りを緩和する目的)。
