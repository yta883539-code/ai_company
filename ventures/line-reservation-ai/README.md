...
- conversation-samples-test-cases.md: LLMシステムプロンプト草案・JSONリトライ設計を検証するための会話サンプル(正常系4件・崩れ系13件)のテストケース設計(2026-07-31 00:59 UTC更新、E10の想定出力をfaq-response-templates.mdの項目別テンプレートに揃えて具体化し、複合質問の分割送信テストケースE13を追加。1応答内でintentが項目ごとに混在しうる場合のJSONスキーマ設計という新課題を発見。机上設計のみで実LLM検証は未着手)
...
- E13で発見した課題: 複合質問で一部項目が未登録の場合、1回の応答内でintent(faq/escalation)が
  項目ごとに混在しうる点をjson-output-retry-fallback.mdの「1応答=1 JSON」前提とどう整合させるか
  (項目ごとの配列で返す等のスキーマ拡張を検討)
