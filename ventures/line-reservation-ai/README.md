...
## 次にやること(候補)
- ヒアリング対象候補(実店舗)のリストアップ"方法"の検討(選定基準・情報源の整理。実際の連絡・依頼はオーナー承認後)
