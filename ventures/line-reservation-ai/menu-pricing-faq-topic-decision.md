...
- (解消済み・character-limit-fallback-design.md参照: メニュー件数が多い店舗で全件列挙が
  LINE Messaging APIの文字数上限(UTF-16コード単位5,000文字)を超える場合、省略・要約は
  行わず、他の未登録FAQ項目と同じ保留文言(`format_faq_unregistered_message()`)へ安全側で
  フォールバックする方針とした。具体的な閾値の妥当性(実運用上もっと手前で気づかせるべきか)は
  実店舗のメニュー件数の実態が見えてから再検討する未解決課題として character-limit-
  fallback-design.md「残る課題」に引き継いだ)
