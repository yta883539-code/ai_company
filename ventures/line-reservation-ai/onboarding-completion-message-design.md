...
- (解消 2026-09-17 21:00 UTC・定例更新: owner-faq-routing-design.md 5節の残課題
  「『FAQ』という単語自体をオーナーが思いつかない可能性がある」への対応として、本
  メッセージの結び文に「よくある質問は『FAQ』と送ると案内する」旨を1文継ぎ足した
  (3トーン共通)。3節「1メッセージ1用件」の原則との整合を保つため、新規段落では
  なく既存の結び文への継ぎ足しとした。本メッセージはオーナー本人にのみ1回だけ送信
  されることが1節で確定しているため、follow時の共用ウェルカムメッセージ(顧客とも
  共用でFAQコマンドの周知先として不適切、詳細はowner-faq-routing-design.md 5節参照)
  より適した周知先として採用した。詳細な検討経緯はowner-faq-routing-design.md 5節参照。)
