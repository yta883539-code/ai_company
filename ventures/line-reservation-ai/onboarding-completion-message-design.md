...
起算点自体は、本メッセージの送信有無とは独立している(申込〈onboarding-guide.mdステップ1〉
起算か、本メッセージの送信〈ステップ3完了〉起算かは未確定。line-reservation-aiには
course-set-pasha/aircon-pashaのような`trial-start-anchor-decision.md`が未整備であり、この
起算点の明文化は本ドキュメントのスコープ外の別課題として残す)。
...
- トライアル起算点(申込時点 or 本メッセージ送信時点)の明文化(`trial-start-anchor-decision.md`
  相当)は別課題として残す。
