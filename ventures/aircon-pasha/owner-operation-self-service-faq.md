...
- ~~course-set-pasha・line-reservation-aiのFAQと同様、本FAQへの導線(オンボーディング完了
  ...
  限定的。~~ → 2026-09-21訂正: 本節の下「次のステップ候補」に記載の通り、フェーズ225
  (owner-faq-routing-design.md)でLINEトークルーム内のコマンド方式導線が実装済みであり、
  本項目は本FAQ作成(フェーズ224)から日を置かず解消されていた。実効性(利用率・削減
  効果)自体は依然未検証であり、その点はsupport-cost-selfservice-reduction.md(フェーズ
  244)に引き継いで検討した。
