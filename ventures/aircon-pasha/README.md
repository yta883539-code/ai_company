...
- フェーズ14(2026-08-10 08:00 UTC): 候補1のエキテン掲載情報・候補3の運営主体「快線屋」の
  ...
  3回連続で会社概要の特定に至らなかった。
- 最終更新: 2026-08-10 08:00 UTC

## 次にやること(候補)

- 候補1は「女性スタッフも在籍」との記述からスタッフ複数名の存在は裏付けられたが合計人数は
  不明なまま。加えてエキテン掲載の所在地(熊本市東区上南部)と会社概要ページの所在地
  (熊本市中央区下通)の相違が新たに判明したため、次回はこれが複数拠点運営(必須条件との
  関係で要確認)なのか単なる情報の新旧なのかを優先的に確認する。
- 候補3は運営主体「快線屋」の会社概要が3回連続で特定できず(公式サイト・商工会議所名簿とも
  該当なし)、必須条件確認が困難になりつつある。次回さらに情報が得られなければ代替候補の
  探索へ切り替える。
