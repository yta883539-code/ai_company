...
- (解消済み 2026-09-07 21:00 UTC・フェーズ198: 本セクション直下のフェーズ184エントリが
  次回以降の課題として残していた(2)(3)が、実際にはいずれも既に解消済みであるにも
  かかわらず本セクションの記載が更新されないまま古い状態で残っていた記載漏れを発見・
  解消した。(2)`stripe_webhook.receive_stripe_webhook()`〈実HTTPエントリポイント〉への
  `cancellation_push_client`/`portal_link_provider`配線はフェーズ186
  (2026-09-04 12:00 UTC)で実装済み(`get_stripe_runtime_dependencies()`は既存の
  `push_client`・`recovery_push_client`と同様、実LINE Push API接続・実Stripeカスタマー
  ポータル接続がオーナー承認待ちのため意図的にNoneのまま渡さない設計)。(3)解約取り消し
  案内メッセージの問い合わせ導線文言の見直しはフェーズ188(2026-09-04 21:00 UTC)で
  実施済み(`SUBSCRIPTION_CANCELLATION_RESCHEDULED_MESSAGE`を、実在しないお問い合わせ先
  記載への言及から、実際に機能するトークルームへの自由文質問という導線案内へ修正)。
  なお(1)は既にフェーズ195で解消済みとして本セクション上部に記載されており、これで
  フェーズ184エントリの次回以降の課題(1)〜(3)は全て解消済みとなった。コード変更は無い
  ドキュメント整合性の確認のみで、venture全体474件全件
  ...
  ドキュメント整合性修正のみで、外部サービスへの公開・アカウント作成・支払い・送信等は
  今回発生していないためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の
  前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする。)
- (解消済み 2026-09-06 00:00 UTC・フェーズ195: 本セクション直下のフェーズ184エントリが
  次回以降の課題(1)として挙げていた「制限モード中の解約予約受理案内の文言整合性
  チェック」は、フェーズ185で文言自体の実装(is_currently_suspended分岐)まで完了して
  おり、本セクションの記載が更新されないまま古い状態で残っていた。フェーズ185が
  新たに残した「文言の妥当性検証」自体もフェーズ195で机上レビューまで実施済み
  (restricted-mode-cancellation-message-copy-review.md参照、実顧客データによる
  再検証のみ引き続き残る)。コード変更は無いドキュメント整合性の確認。)
...
- フェーズ193(2026-09-05 19:00 UTC): フェーズ192「次回以降の課題」として残っていた
  landing-page-copy-draft.mdへの「時給換算」訴求反映に対応した。course-set-pashaの
  landing-page-copy-draft.md(フェーズ109相当)と同じ構成で、料金・トライアル訴求
  セクション直後に「『時給換算』訴求(補足コピー案)」節を新設し、
  content-generation-time-estimate.mdの`minutes(n) = 8 + 5n`(1台のみの場合13分)と
  pricing-plan.mdの各プラン上限生成回数(スモール40回/スタンダード90回/繁忙期対応150回)
  を掛け合わせた時給換算コピー案(スモール約344円/時・スタンダード約307円/時・繁忙期
  対応約276円/時)を追加した。本venture固有のpricing-plan.md運用(「1メモ=1台=1回」)を
  踏まえ、複数台まとめて送る場合の追加按分(`ADDITIONAL_MINUTES_PER_UNIT`)は上限回数の
  試算には含めず注記でのみ言及するにとどめた。content-generation-time-estimate.md
  「残課題」の該当項目を解消済みに更新した。コード変更は無く、ドキュメントのみの更新
  (数値試算・コピー草案の追加)のため、venture全体のテスト件数に変化は無い(既存の
  474件全件パスは前フェーズ確認済みのまま変化なし)。承認不要なコンテンツ草案作成のみで、
  外部サービスへの公開・アカウント作成・支払い等は今回発生していないため
...
- フェーズ194(2026-09-05 22:00 UTC): 未走査の設計docの残課題棚卸しを行った結果、
  ...
  ためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の前進、または
  引き続き未走査の設計docの残課題棚卸しを優先候補とする。
- フェーズ195(2026-09-06 00:00 UTC): subscription-cancellation-scheduled-message-
  suspension-consistency-design.md(フェーズ185)7節の残課題「制限モード中の解約予約
  受理案内メッセージの具体的な文言は実顧客の問い合わせ実績が無く妥当性検証が必要
  (course-set-pashaフェーズ157と同じ課題)」に対応した。実顧客の反応データが得られない
  現段階でできる範囲として、現行文言(フェーズ185実装)を4つの観点(理解しやすさ・
  行動の明確さ・トーンの煽り過ぎ回避・既存通知文言との一貫性)から自己点検し、代替候補
  2案(生成停止を先に述べる案・2手続きを番号付きで列挙する案)と比較した上で、現行文言を
  維持する結論とその理由を整理した(restricted-mode-cancellation-message-copy-review.md
  新規作成)。コード変更は無く、venture全体のテスト件数に変化は無い(既存の474件全件は
  前フェーズ確認済みのまま変化なし)。実顧客の実測データによる再検証は引き続き未解消の
  課題として残る(course-set-pashaフェーズ157の同種課題も同様に未検証のまま)。承認不要な
...
- フェーズ197(2026-09-06 21:00 UTC): course-set-pashaフェーズ196(2026-09-06 20:00 UTC)が
  ...
  今回発生していないためpending-approval.mdへの追記なし。次回は他venture・アイデア領域の
  前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする。
- フェーズ198(2026-09-07 21:00 UTC): 未走査の設計docの残課題棚卸しの一環として、
  「次にやること(候補)」セクション冒頭に残っていたフェーズ184エントリを確認した
  ところ、次回以降の課題として挙げていた(1)〜(3)のうち、(1)は既にフェーズ195で解消済み
  として本セクション上部に記載されていたが、(2)`stripe_webhook.receive_stripe_webhook()`
  への`cancellation_push_client`/`portal_link_provider`配線(フェーズ186で解消済み)・
  (3)解約取り消し案内メッセージの問い合わせ導線文言の見直し(フェーズ188で解消済み)の
  2点は、本文中の履歴(フェーズ186・188)では解消済みと記録されているにもかかわらず、
  本セクション側の記載が更新されないまま古い状態で取り残されていた記載漏れだった
  (フェーズ187・191等で繰り返し見つかっている「実装フェーズが先行し集約セクションの
  更新が追いつかない」同種のパターンの再発)。本セクション冒頭に解消済みである旨・
  対応フェーズと内容の要約を追記した。コード変更は無いドキュメント整合性の確認のみで、
  venture全体474件全件(`python3 -m unittest discover -s prototype -p "test_*.py"`)・
  schema検証9件(`python3 schema/validate_test_cases.py`)パスを確認した(変更前と同じ
  結果)。承認不要なドキュメント整合性修正のみで、外部サービスへの公開・アカウント作成・
...
- フェーズ202(2026-09-11 12:00 UTC): 未走査の設計docの残課題棚卸しの一環として
  ...
  領域の前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする。
- フェーズ203(2026-09-11 15:00 UTC): 未走査の設計docの残課題棚卸しの一環として、
  blocked-but-billing-detection-design.md「4. 未着手のまま残る課題」末尾に残っていた
  「line-reservation-aiにも同種の未着手課題(unfollow時のuser_profile扱い)が残っているため、
  次回以降の展開候補とする」という記載(フェーズ167、2026-08-28頃作成)を確認したところ、
  実際にはline-reservation-ai側で既にフェーズ続き176(2026-09-02 23:02 UTC)にて対応済み
  であることが判明した。line-reservation-aiは本venture固有の`is_following`(user_id単位)を
  そのまま持ち込むのではなく、当該venture固有の事情(LINE公式アカウントを一般顧客とオーナー
  自身の両方がフォローする構造)を踏まえて`owner_is_following`(store_id単位)という別設計へ
  翻案したうえで実装済みであり、本行が想定していた「本venture固有の設計をそのまま持ち込める
  か」という懸念自体は該当せず、対象venture側で個別に適した設計へ翻案する形で既に解消されて
  いた(実装から掲載まで約1週間、doc側が追随できずに残っていた「実装フェーズが先行し集約
  セクションの更新が追いつかない」同種のパターンの再発)。該当箇所を解消済みの旨(対応
  フェーズ・実装内容の要約)に更新した。コード変更は無く、venture全体475件全件(`python3 -m
  unittest discover -s prototype -p "test_*.py"`)・schema検証9件
  (`python3 schema/validate_test_cases.py`)いずれもパスを確認した(変更前と同じ結果)。
  承認不要な設計doc記載の整合性修正のみで、外部サービスへの公開・アカウント作成・支払い・
  送信等は今回発生していないためpending-approval.mdへの追記なし。次回は他venture・アイデア
  領域の前進、または引き続き未走査の設計docの残課題棚卸しを優先候補とする。
- フェーズ204(2026-09-11 19:00 UTC): course-set-pashaフェーズ204(2026-09-11 18:00 UTC)で
  「セッター複数プラン」の想定顧客像「複数店舗を運営する事業者」が、実装
  (`user_profile/{user_id}`単位=1 user_id=1契約)では複数の異なるLINEアカウントでの
  共同利用をサポートしていないというギャップが発見されたのを受け、本ventureのpricing-plan.md
  にも同種の文言(繁忙期対応プランの想定顧客像「複数職人を抱える小規模法人」)が無いか横断
  確認したところ、同じパターンのギャップが見つかった。本venture固有の事情(単方向バッチ処理、
  生成される報告文・案内文の宛先は依頼者(顧客)であり送信した職人個人を宛てた文面ではない
  ため「誰が送ったか」を区別する業務要件が無い)を踏まえて検討した結果、course-set-pashaと
  同じ結論(MVPでは複数user_id対応を見送り、「1つのLINEアカウントを事業所内で複数職人が
  共有運用する」前提と定義)に至り、multi-technician-shared-usage-design.mdを新規作成した
  うえで、pricing-plan.mdの繁忙期対応プラン行の想定顧客像に共有運用前提の補足を追記した
  (コード変更は不要)。venture全体475件全件(`python3 -m unittest discover -s prototype -p
  "test_*.py"`)・schema検証9件(`python3 schema/validate_test_cases.py`)いずれもパス
  (変更前と同じ結果)を確認した。承認不要なアイデア追加・設計文書作成・既存ドキュメントの
...
- フェーズ211(2026-09-13 19:00 UTC): llm-system-prompt-draft.md「次の課題」に残っていた、
  ...
  前進を優先候補とする。
- フェーズ212(2026-09-13 23:00 UTC): llm-system-prompt-draft.mdの「構造化出力の方針」節に
  取り残されていた記載漏れを修正した。同節の見出しは「(方針のみ、スキーマファイル未作成)」、
  節末の一文は「実際のJSON Schemaファイル(schema/output.schema.json相当)の作成は次回以降の
  課題とする」のままだったが、実際には構造化出力スキーマ(schema/output.schema.json)は
  フェーズ97相当(「次の課題」節记載の2026-08-21解消済み事項)で既に作成済みであり、
  以降のフェーズ(6a/6bの`status`分岐追加、G1〜G6サンプル整備等)も一貫してこのスキーマ
  ファイルを前提に進められていた。見出し・節末の一文をいずれも「解消済み」に訂正し、
  実装済みである旨と参照先(「次の課題」節)を明記した。コード変更は無く、念のため回帰確認
  として`python3 -m unittest discover -s prototype -p "test_*.py"`(485件)・`python3
  schema/validate_test_cases.py`(15件)を実行し、いずれも変更前と同じ結果でパスすることを
  確認した。承認不要なドキュメント整合性修正・回帰確認のみで、外部サービスへの公開・
  アカウント作成・支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。
  次回は実LLM接続後に残る検証課題、または他venture・アイデア領域の前進を優先候補とする。
- フェーズ213(2026-09-14 06:00 UTC): market-research.mdの「未検証事項(次回以降の課題)」に
  取り残されていた記載漏れを解消した。同節の2点目「依頼者向け完了報告メッセージの実例
  (公開されている業者ブログ・SNS投稿等)の観察は未実施」という記載は、実際にはフェーズ相当の
  作業として2026-08-09 18:00 UTCにsns-blog-example-observation.mdで既に実施済み(WebSearchで
  実在業者の公開情報を観察し、動作確認への言及・防カビコートの効果持続期間の留保表現・
  推奨頻度の粒度・自己分解洗浄リスクの注意喚起の4点をllm-system-prompt-draft.mdの
  厳守事項3・4へ反映済み)であり、course-set-pashaフェーズ208・kura-pashaフェーズ114・
  line-reservation-aiフェーズ続き225で見つかった記載漏れと同種の問題だった。該当箇条書きを
  「解消済み」に訂正し、参照先(sns-blog-example-observation.md)と、個人事業主・独立系業者に
  絞ったSNS投稿の実例自体は見当たらず業界大手・情報サイトの解説記事中心の観察に留まった
  という限界(course-set-pashaのsns-tone-research.mdと同様)は未解消のまま残る旨を明記した。
  コード変更は無く、回帰確認として`python3 -m unittest discover -s prototype -p
  "test_*.py"`(485件)・`python3 schema/validate_test_cases.py`(15件)を実行し、いずれも
  変更前と同じ結果でパスすることを確認した。承認不要なドキュメント整合性修正・回帰確認のみで、
  外部サービスへの公開・アカウント作成・支払い・送信等は今回発生していないため
  pending-approval.mdへの追記なし。次回は市場調査の残課題(BtoB経由の需要調査等)、
  または他venture・アイデア領域の前進を優先候補とする。
- フェーズ214(2026-09-14 08:00 UTC): market-research.mdの「未検証事項(次回以降の課題)」
  3点目「賃貸物件の管理会社・不動産オーナー経由での定期清掃契約等、BtoB経由の需要が
  あるかは未調査」について、WebSearchで一次調査を行った。(1)退去時のエアコンクリーニング
  費用は国交省の原状回復ガイドラインに沿えば通常損耗として大家・管理会社側負担となるのが
  一般的であり、管理会社自身が支払主となるBtoB需要の存在自体は裏付けられた。(2)ただし
  管理会社は既に「提携業者」を確保している運用が一般的で、個人事業主が新規参入するには
  既存業者からの切替コストを超える提案が必要と見られる。(3)複数戸まとめた「一括契約」の
  具体的な契約形態・単価水準は検索結果からは確認できず未解消のまま残る。(4)示唆として、
  管理会社向けには出力1(完了報告メッセージ)の宛先・内容を「原状回復記録・オーナー説明
  資料」向けに翻案できる可能性があるが、通常損耗の該当性判断自体は法的判断であり
  代行しない厳守事項との整合が必要な点をMVP設計上の新たな検討候補として明記した。
  market-research.mdの該当箇条書きを更新のみで、コード変更は無く、念のため回帰確認として
  `python3 -m unittest discover -s prototype -p "test_*.py"`(485件)・`python3
  schema/validate_test_cases.py`(15件)を実行し、いずれも変更前と同じ結果でパスすることを
  確認した。承認不要なWebSearchによる市場調査・ドキュメント更新のみで、外部サービスへの
...
- フェーズ216(2026-09-14 13:00 UTC): market-research.mdフェーズ214が新たな検討候補として
  残していた「管理会社向け出力1(作業完了報告メッセージ)の翻案が可能か」というMVP設計課題に
  対応した。btob-management-company-report-variant-design.mdを新規作成し、出力1の宛先
  (recipient: tenant/management_company)を区別する設計とした。管理会社向けは文体を事務的な
  ものに変更したうえで、原状回復における費用負担区分(通常損耗か否か)の法的判定は行わない
  旨の固定ボイラープレート文言を必ず付す方針とし、厳守事項1(冷媒・電気系統への専門的助言を
  しない)・厳守事項2(効果・効能を推測で付け足さない)との整合を確認した(新設・厳守事項8の
  たたき台として位置づけ)。market-research.mdの該当箇条書きを更新し参照先を明記した。
  schema/output.schema.json・llm-system-prompt-draft.mdへの正式な反映、出力2(お手入れ案内)の
  管理会社宛での扱いは未着手のまま次回以降の課題として残した。コード変更は無く、回帰確認
  として`python3 -m unittest discover -s prototype -p "test_*.py"`(485件)・`python3
  schema/validate_test_cases.py`(15件)を実行し、いずれも変更前と同じ結果でパスすることを
  確認した。承認不要な設計文書作成・ドキュメント更新のみで、外部サービスへの公開・
  アカウント作成・支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。
...
- フェーズ217(2026-09-14 15:00 UTC): フェーズ216が次回以降の課題として残していた
  ...
  なし。
- フェーズ218(2026-09-14 16:00 UTC): フェーズ217・btob-management-company-report-variant-
  design.mdが次回以降の課題として残していた2点に対応した。(1)出力2(お手入れ案内)を管理
  会社宛でどう扱うかは、省略せず入居者向けと同一内容のまま生成する方針で確定した(出力2の
  内容はそもそも入居者本人が実施する行為への案内であり、管理会社が入居者へ転送・同封する
  運用と自然に整合するため、省略すると入居者側の有用な情報が失われるデメリットの方が大きい
  こと、宛先別に翻案しようとすると入力メモに無い判断を追加するリスクが生じる一方で得られる
  価値は限定的〈出力1と異なり事務的な文体である必要性が薄い〉ことを理由とする)。このため
  schema/output.schema.jsonのcare_guideにrecipient相当のフィールドを追加する必要は無いと
  判断した。(2)「宛先:管理会社」等のメモ末尾表記の具体的なパース方法(表記ゆれの許容範囲)は、
  course-set-pasha・line-reservation-ai等の既存venture群と同じくコード側の文字列パースでは
  なくLLMの解釈に委ねる設計を踏襲し、llm-system-prompt-draft.md厳守事項9に`management_
  company`と判定すべき具体的な表記例(全角/半角コロン、「管理会社宛」「管理会社宛て」等の
  「宛」表記)と、対象が一意に定まらない曖昧な表記や宛先の明示が無い場合は安全側のデフォルト
  `tenant`として扱う旨(厳守事項1・2と同じ「推測で補わない」原則の踏襲)を追記した。
  btob-management-company-report-variant-design.md「未反映の実装項目」・llm-system-prompt-
  draft.md「次の課題」双方に解消済みの記録を追記し、schema/output.schema.jsonのcare_guide
  周辺の説明文にも今回の設計判断(recipientフィールド不要)を追記した。いずれもコード変更は
  無く、回帰確認として`python3 -m unittest discover -s prototype -p "test_*.py"`(491件)・
  `python3 schema/validate_test_cases.py`(17件)を実行し、いずれも変更前と同じ結果でパス
  することを確認した。承認不要な設計文書・システムプロンプト草案・スキーマ説明文の更新の
  みで、外部サービスへの公開・アカウント作成・支払い・送信等は今回発生していないため
  pending-approval.mdへの追記なし。表記ゆれ吸収の実際の判定精度は実LLM接続後(オーナー
  承認待ち)の検証課題として残る。次回は他venture・アイデア領域の前進、または実LLM接続後の
  検証課題の棚卸しを優先候補とする。
- フェーズ219(2026-09-14 20:00 UTC): market-research.md・btob-management-company-report-
  variant-design.md(フェーズ214〜218)で需要の存在自体は裏付けられていたが料金設計が
  未着手のまま残っていた管理会社向け(recipient=management_company)のBtoB一括契約プラン
  について、pricing-plan.mdに仮設計を追加した。個人向けプラン(「1メモ送信=1回」課金)とは
  分け、管理会社の予算感に合わせた「戸(物件)」単位の課金案(管理会社ライト: 月9,800円・
  20戸まで、管理会社スタンダード: 月24,800円・60戸まで、いずれも超過分は従量課金)を
  月間退去精算戸数の概算(管理戸数×年間退去率20%÷12ヶ月)から仮置きした。施工業者向け
  個人プランとの二重課金を避けるため、施工業者が個人向けプランで契約し出力を管理会社へ
  転送するか、管理会社自身が管理会社向けプランで契約し提携業者からのメモ入力を受け付ける
  かのいずれか排他的な契約形態を想定する旨も明記した。課金単位(戸数か生成回数か)・単価
  水準はいずれも未検証であり、実際の管理会社へのヒアリング(2026-08-23 04:00 UTC付で
  pending-approval.mdに記録済みの承認待ち事項)なしには確定できない旨を「未検証」として
  明記した。コード変更は無く、回帰確認として`python3 -m unittest discover -s prototype -p
  "test_*.py"`(491件)・`python3 schema/validate_test_cases.py`(17件)を実行し、
  いずれも変更前と同じ結果でパスすることを確認した。承認不要な料金設計ドキュメントの追加
  のみで、外部サービスへの公開・アカウント作成・支払い・送信等は今回発生していないため
  pending-approval.mdへの追記なし。次回は他venture・アイデア領域の前進、または本プラン案の
  管理会社ヒアリング後の見直しを優先候補とする。
- フェーズ220(2026-09-15 00:00 UTC): market-research.mdの未検証事項(3)「一括契約」
  「まとめて複数戸を定期契約」の具体的な契約形態・単価水準について、WebSearch「エアコン
  クリーニング 管理会社 複数戸 一括契約 年間契約 単価」「賃貸 退去 エアコンクリーニング
  管理会社 提携業者 契約 料金表 複数台」で追加調査した。本サービス(下書き生成)自体の
  複数戸一括契約事例は今回も確認できなかったが、隣接領域(エアコンクリーニング施工そのもの)
  では複数台同時依頼時に2台目以降が1,000〜3,000円程度割引される料金体系が一般的という
  記述を複数の情報源で確認し、「まとまった台数・戸数ほど1件あたり単価が下がる」という
  業界慣行がpricing-plan.mdの管理会社向けプラン階層(ライト300円/戸→スタンダード250円/戸)
  の方向性と整合することをmarket-research.mdに追記した。ただし下書き生成サービス自体の
  単価水準を裏付けるものではなく傍証に留まるため、単価水準の確定には引き続き実際の管理
  会社・独立系業者へのヒアリング(オーナー承認待ち)が必要という結論に変更は無い。コード
  変更は無く、回帰確認として`python3 -m unittest discover -s prototype -p "test_*.py"`
  (491件)・`python3 schema/validate_test_cases.py`(17件)を実行し、いずれも変更前と
  同じ結果でパスすることを確認した。承認不要な市場調査ドキュメントの追記のみで、外部
...
- フェーズ231(2026-09-19 01:00 UTC): llm-system-prompt-draft.md「次の課題」が厳守事項6b
  (2026-09-12新設、有料プラン開始意図検知)の既知の限界として残していた(iii)雑談と(iv)
  判断不能の切り分けについて、フェーズ230でG8として固定した6a(iii)側と対になる6b(iii)側の
  具体例が未着手のままだったことに気付き、schema/validate_test_cases.pyに
  G9_busy_grumble_not_checkout_intentとして新規追加した。入力メモ想定:「今月は忙しくて
  プラン変更とか考える余裕がない」+実際の分解洗浄内容。「プラン変更」の語を含むが申し込み・
  開始意図には触れない雑談の域を出ない表現であるため、status=generatedとして通常どおり
  作業完了報告・お手入れ案内を生成し、checkout_noticeはNoneのままとするのが期待動作。
  output-samples-validation.mdの一覧表・件数(18件→19件)を更新した。schema検証19件
  (18件→19件、`python3 schema/validate_test_cases.py`)・venture全体513件(コード変更は
  無く回帰確認、`python3 -m unittest discover -s prototype -p "test_*.py"`)いずれもパス
  を確認した。承認不要なドキュメント・サンプル追加のみで、外部サービスへの公開・アカウント
  作成・支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。実LLMが
  6b(iii)の繁忙の愚痴を実際に有料プラン開始意図と誤検知しないかの検証は、6a側と同様に
  実LLM接続後(オーナー承認待ち)の課題として残る。
...
- 最終更新: 2026-09-21 17:00 UTC(フェーズ246: sns-blog-example-observation.mdの
  ...
  schema検証25件いずれもパス)
- フェーズ247(2026-09-21 22:00 UTC定例更新): 他3venture(course-set-pasha・kura-pasha・
  line-reservation-ai)が本日中に前進していた一方、本ventureは17:00 UTC(フェーズ246)を
  最後に最も手薄になっていたため今回選び、onboarding-guide.mdステップ1・2の記載を実際の
  設計・実装と突き合わせて棚卸しした。ステップ2「LINE公式アカウント開設・チャネルアクセス
  トークン取得を踏襲する」という記載が、フェーズ106以前(連携コード方式確定前)の初期記述の
  まま残っており、フェーズ107(user-account-linking-design.md)で確定した「業者は公式
  アカウントを友だち追加するだけでよく、独自のMessaging APIチャネル開設・チャネルアクセス
  トークン取得は不要」という実際の設計(共有の単一公式アカウント+連携コード方式、
  course-set-pasha・kura-pashaと同じ)と食い違ったまま4フェーズ以上未修正だった記載漏れを
  発見した。line-reservation-aiは「店舗ごとに1つLINE公式アカウントを発行する」前提
  (store-id-resolution-and-owner-identity-design.md)のため本venture向けにそのまま踏襲した
  記述が実態に合わなくなっていたと判断し、ステップ1(サンクスページでの連携コード表示の
  明記)・ステップ2(友だち追加+連携コード送信のみでよい旨への書き換え、line-reservation-ai
  との違いの明記)・「次のステップ候補」(スクリーンショット対象がLINE Developersコンソール
  操作ではなく友だち追加画面である旨への訂正)を修正した。実装(prototype/
  user_id_linking.py・cloud_function_webhook.py)自体はフェーズ107〜113で既に連携コード方式
  で作られておりコード変更は無く、回帰確認としてventure全体515件(`python3 -m unittest
  discover -s prototype -p "test_*.py"`、変更前と同数)・schema検証25件(`python3
  schema/validate_test_cases.py`、変更前と同じ結果)いずれもパスを確認した。承認不要な
  ドキュメント記載訂正のみで、外部サービスへの公開・アカウント作成・支払い・送信等は
  今回発生していないためpending-approval.mdへの追記なし。
- 最終更新: 2026-09-21 22:00 UTC(フェーズ247: onboarding-guide.mdステップ2が、フェーズ107で
  確定した「共有公式アカウント+連携コード方式」より前の初期記述〈業者が自らLINE公式
  アカウント・チャネルアクセストークンを取得する〉のまま4フェーズ以上未修正だった記載漏れを
...
- フェーズ254(2026-09-23 02:00 UTC定例更新): フェーズ253が残していた、
  interview-rehearsal-script.mdへの質問16(複数職人共有運用の確認)反映に対応した。
  フェーズ245での質問15反映と同じ方針(冒頭に反映履歴の追記、想定タイムテーブル・
  オープニング読み上げ台本・質問ごとの補足ト書き・リハーサルでの確認ポイントの4箇所へ
  反映)を踏襲した。質問16は「複数職人を抱える小規模法人」該当者のみに聞く条件付き設問の
  ため、Q11(SNS絵文字質問)・Q12(フランチャイズ向け設問の代替)と同様に事前確認の一言を
  挟む運用とし、ロングリスト化済みの候補1・3・7・10・12(独立系の個人事業主)・候補13・14
  (フランチャイズ加盟の個人事業主)はいずれも非該当のため現行台本では「該当なしのため
  スキップ」扱いとなる点、該当候補(複数メニュー展開のハウスクリーニング業者)は
  candidate-longlist-draft.mdで継続探索中(2026-08-24時点で一旦打ち切り、初回コンタクト
  承認後のヒアリング過程で確認する運用)である点を明記した。想定タイムテーブルの
  質問数表記を「全15問」から「基本15問+条件付き1問(Q16)」に更新し、Q16が該当する
  場合の時間上振れ(30秒程度)も明記した。customer-interview-design.md側の「未検証の
  仮説」も本対応を反映して更新した。コード変更は無く、回帰確認としてventure全体519件
  (`python3 -m unittest discover -s prototype -p "test_*.py"`、変更前と同数)・
  schema検証25件(`python3 schema/validate_test_cases.py`、変更前と同じ結果)いずれも
  パスを確認した。承認不要なドキュメント更新のみで、外部サービスへの公開・アカウント
  作成・支払い・送信等は今回発生していないためpending-approval.mdへの追記なし。
  これによりmulti-technician-shared-usage-design.md(フェーズ204)「残る課題」に
  残っていた顧客ヒアリング関連の対応はすべて完了した。次回は他venture・アイデア領域の
  前進を優先候補とする。
- 最終更新: 2026-09-23 02:00 UTC(フェーズ254: フェーズ253が残していたinterview-
  rehearsal-script.mdへの質問16〈複数職人共有運用の確認〉反映に対応。想定タイムテーブル・
  オープニング台本・補足ト書き・確認ポイントの4箇所へ反映し、質問数表記を「基本15問+
