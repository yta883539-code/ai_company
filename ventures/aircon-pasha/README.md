...
- フェーズ49(2026-08-16 00:00 UTC): フェーズ48の申し送りだった候補11(エアクリ)の
  ...
  整合するかを次回検討課題とした。
- 最終更新: 2026-08-16 00:00 UTC

## 次にやること(候補)

- 候補11(エアクリ)自身の所在エリア・代表者名・スタッフ数の特定(company.html本文の
  直接確認は引き続きegress制約で不可のため、問い合わせフォーム記載や第三者記事等の
  間接情報を試みる)。進展がなければ保留に回し独立系新規候補探索(残り2〜3件)を優先する
  方針への転換を判断する。「技術指導・研修事業を兼業する候補」の扱いを
  interview-candidate-selection-criteria.mdに論点として追記するかどうかも次回検討する。
  (詳細はcandidate-longlist-draft.md「第二十七弾」参照)
