...
# 明記する、design 2節の安全設計)。
# owner-faq-routing-design.md 5節の残課題(「FAQ」コマンドの存在をどう周知するか)に対応し、
# フェーズ226でFAQコマンドの案内文を追記した(first-generation-self-check-design.md
# 「残課題」参照)。この初回生成時セルフチェック案内は全業者が生涯に一度は必ず受け取る
# 唯一確実な導線であるため、既存の固定文言(セルフチェック依頼)を変更せず末尾に1文
# 追記するのみとし、新たな送信トリガー・状態フィールドの追加は行わない。
